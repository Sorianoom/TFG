...
\hline
\end{tabular}
\caption{Conjuntos de evaluación a nivel de flujo y su tamaño (fuente: resúmenes \texttt{flow\_level\_detection\_summary\_*} y \texttt{generalization\_summary\_*}).}
\label{tab:exp_config}
\end{table}

Los ficheros principales de entrada son las ventanas extraídas de UGR'16 (Capítulo~\ref{cap:algoritmos}). Los de salida son los resúmenes \texttt{flow\_level\_detection\_summary\_v3.csv} y \texttt{flow\_level\_detection\_summary\_v5\_integrated.csv} (conjunto principal de estudio), los \texttt{generalization\_summary\_v5\_integrated\_*.csv} (generalización) y \texttt{ml\_baseline\_summary.csv} (línea base). El detalle por traza queda en los ficheros \texttt{flow\_level\_detection\_results\_*}.
...
\hline
\end{tabular}
\caption{Detección binaria (ataque/normal) en \texttt{august.week1}. Fuente: \texttt{flow\_level\_detection\_summary\_v3.csv} y \texttt{\_v5\_integrated.csv}. $^{*}$El fichero de la v3 no almacena el F1, que se obtiene de su precisión y recall.}
\label{tab:exp_binaria}
\end{table}

Las dos versiones separan bien el tráfico de ataque del tráfico normal, con un recall de 0,991 en los dos casos. Aun así, hay un matiz: en el conjunto principal de estudio, la v5 obtiene una precisión ligeramente \emph{menor} que la v3 (0,926 frente a 0,930). La causa está documentada en el propio resumen: el tercer pase SSH de la v5 genera 8\,389 predicciones adicionales que, en esta semana, se contabilizan como falsos positivos (campo \texttt{SSH\_PASE3 override\_fp}), porque \texttt{august.week1} apenas contiene escaneo SSH etiquetado. Dicho de otra forma, el pase que permite recuperar el escaneo SSH tiene un coste en precisión en aquellas semanas donde ese ataque no aparece etiquetado. Esa contrapartida se compensa en \texttt{april.week2}, como se verá en la sección de generalización.
...
\caption{Comparacion del rendimiento binario de las versiones v3 y v5 y efecto del contexto global en la deteccion de \texttt{anomaly-sshscan}.}
\label{fig:v3_vs_v5}
\end{figure}

La v5 no es un detector perfecto. Su valor está en que mantiene el rendimiento de la v3 en el núcleo de ataques y, además, recupera una familia que la v3 no detectaba en absoluto, a costa de algún falso positivo en las semanas sin ese ataque. La v4, en cambio, exploró un reparto de confianza en la familia de botnet que no mejoró de forma concluyente, por lo que no se adopta.
...
\hline
\end{tabular}
\caption{Resultados por familia en \texttt{august.week1} (derivados de \texttt{flow\_level\_detection\_summary\_v5\_integrated.csv}). El escaneo SSH se evalúa realmente en \texttt{april.week2} (Tabla~\ref{tab:exp_sshscan}).}
...
\caption{F1 por familia de ataque para la version v5 del clasificador.}
\label{fig:f1_familia}
\end{figure}

El comportamiento por familia es el siguiente:
...
\hline
\end{tabular}
\caption{Escaneo SSH detectado por la v5 en \texttt{april.week2} (fuente: \texttt{generalization\_summary\_v5\_integrated\_april\_week2.csv}).}
...
Como referencia secundaria, se entrenaron varios modelos clásicos de aprendizaje automático sobre una muestra equilibrada por clases: regresión logística, $k$ vecinos más cercanos (KNN), máquinas de vectores soporte (SVM) y un perceptrón multicapa (MLP). También se entrenó un Random Forest, aunque se deja \textbf{fuera de la comparación principal} por las razones que se explican más adelante. Estos modelos se utilizan como \textbf{línea base de comparación}, no como sustituto del clasificador contextual \cite{buczak2016survey}. La Tabla~\ref{tab:exp_ml} resume su rendimiento.
...
\hline
\end{tabular}
\caption{Línea base de ML clásico: F1 macro sobre las 8 clases originales (incluían \texttt{anomaly-spam} y \texttt{blacklist}) frente al cálculo sobre las clases activas. Fuente: \texttt{ml\_baseline\_summary.csv} y \texttt{ml\_baseline\_summary\_active\_families.csv}. El F1 macro es la media del F1 por clase.}
\label{tab:exp_ml}
\end{table}
...
Como el F1 macro es la media del F1 por clase, recalcularlo sobre las clases activas no altera las métricas por clase: simplemente deja de promediar las clases excluidas. El cálculo se realiza con el script \texttt{recompute\_baseline\_no\_spam.py} sobre el resumen por clase, sin reentrenar ni sobrescribir los CSV originales, y se guarda en \texttt{ml\_baseline\_summary\_active\_families.csv}. La Tabla~\ref{tab:exp_ml} muestra el efecto: al dejar fuera dos clases en las que varios modelos rendían mal (en especial la regresión logística y la SVM), el F1 macro sube en todos los modelos. Por ejemplo, la regresión logística pasa de 0,615 a 0,760.

El modelo Random Forest se deja fuera de esta comparación principal. En las pruebas alcanzaba métricas casi perfectas, con un F1 macro en torno a 0,98. No es un resultado que se oculte, pero se interpreta con cautela: en una muestra equilibrada y con características derivadas, puede reflejar en parte particularidades del propio conjunto de datos más que una capacidad real de generalización \cite{sommer2010outside}. Por eso se documenta, pero no se usa como referencia principal de la comparación. Aun así, sus resultados siguen guardados en los ficheros del experimento.
...
\hline
\end{tabular}
\caption{Detección binaria de la v5 por semana (fuente: \texttt{flow\_level\_detection\_summary\_v5\_integrated.csv} y \texttt{generalization\_summary\_v5\_integrated\_*.csv}).}
...
Los resultados deben interpretarse teniendo en cuenta las limitaciones de la Tabla~\ref{tab:exp_amenazas}.
...
\hline
\textbf{Amenaza} & \textbf{Descripción} \\
\hline
Dependencia de UGR'16 & Toda la evaluación se hace sobre un único dataset. Los resultados pueden no trasladarse a otras redes. \\
\hline
Desbalance extremo & El tráfico normal domina ampliamente, lo que obliga a interpretar las métricas con cuidado y descarta la exactitud como medida principal. \\
\hline
Etiquetas disponibles & La evaluación depende de las etiquetas del dataset. Puede ocurrir que algunos flujos marcados como \emph{background} tengan una estructura anómala o parecida a la de un ataque, aunque no estén etiquetados como tal. En ese caso se contabilizan como falsos positivos. \\
\hline
Ausencia de payload & NetFlow no incluye contenido, lo que limita de forma estructural la detección de algunas familias. \\
\hline
Familias con pocas muestras & El escaneo SSH tiene muy pocas trazas en \texttt{august.week1} (44) y \texttt{august.week2} (5), por lo que su evaluación se concentra en \texttt{april.week2}. \\
\hline
Sesgo de diseño & Las reglas se formularon observando ventanas del propio dataset, lo que puede favorecer los resultados sobre esos mismos datos. \\
\hline
Generalización limitada & Solo se ha probado en otras semanas de UGR'16, no en tráfico de una red distinta. El escaneo UDP no se ha podido evaluar fuera del conjunto principal de estudio. \\
\hline
No equivale a producción & La evaluación se hace sobre ventanas y capturas, no sobre un flujo continuo en tiempo real. \\
\hline
\end{tabular}
\caption{Principales amenazas a la validez de los resultados.}
\label{tab:exp_amenazas}
\end{table}
