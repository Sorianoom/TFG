...
En la práctica, el conjunto de datos se maneja como \textbf{archivos CSV de trazas NetFlow}, en los que cada fila es un flujo y cada columna un campo de sus metadatos. Las trazas de UGR'16 siguen un formato de \textbf{13 columnas} sin cabecera, derivado de la exportación con herramientas tipo \emph{nfdump}. La Tabla~\ref{tab:csv_netflow_estructura} resume ese formato con el campo, su tipo de dato y su dominio o interpretación.
...
\hline
\textbf{Campo} & \textbf{Tipo} & \textbf{Dominio o interpretación} \\
\hline
Marca temporal & Fecha y hora & Inicio del flujo (formato \texttt{AAAA-MM-DD hh:mm:ss}). \\
\hline
Duración & Numérico (segundos) & Duración del flujo, con decimales (por ejemplo, 0,272). \\
\hline
IP de origen & Texto (dirección IP) & Dirección de la máquina que inicia la comunicación. \\
\hline
IP de destino & Texto (dirección IP) & Dirección de la máquina contactada. \\
\hline
Puerto de origen & Entero & 0--65535. Puerto desde el que se emite. \\
\hline
Puerto de destino & Entero & 0--65535. Identifica el servicio (por ejemplo, 22 para SSH). \\
\hline
Protocolo & Categórico & Protocolo de transporte (TCP, UDP, etc.). \\
\hline
Banderas & Categórico (cadena) & Banderas TCP activadas (por ejemplo, \texttt{.AP.SF}). Poco informativas en UDP. \\
\hline
Campo auxiliar 1 & Entero & Campo del formato (estado de reenvío). No se usa en el análisis. \\
\hline
Campo auxiliar 2 & Entero & Campo del formato (tipo de servicio). No se usa en el análisis. \\
\hline
Paquetes & Entero & Número de paquetes del flujo. \\
\hline
Bytes & Entero & Volumen de datos del flujo, en bytes. \\
\hline
Etiqueta & Categórico & Clase real de la traza (\emph{background} o tipo de ataque). Se usa solo para evaluar. \\
\hline
\end{tabular}
\caption{Estructura general del CSV de trazas NetFlow utilizado en el proyecto.}
\label{tab:csv_netflow_estructura}
\end{table}
...
\section{Material entregado y reproducibilidad}
\label{sec:herr_entrega}

El código fuente del proyecto (scripts de preparación, extracción de ventanas, clasificación y comparación, junto con la aplicación web) se entrega aparte, como repositorio o material adjunto, y no se reproduce en esta memoria. La memoria solo incluye \textbf{pseudocódigo} cuando ayuda a explicar el método (Capítulo~\ref{cap:algoritmos} y Apéndice~\ref{ap:pseudocodigo}). Los datasets originales completos no se adjuntan por su tamaño, ya que UGR'16 ocupa en torno a 80~GB y es de acceso público. Sí pueden entregarse una \textbf{muestra representativa} y los ficheros derivados ligeros necesarios para reproducir los ejemplos, sin subir ficheros muy grandes. En todos los casos, los datos entregados deben entenderse como archivos CSV con las columnas descritas en la Tabla~\ref{tab:csv_netflow_estructura}.
